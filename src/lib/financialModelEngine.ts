...
export function periodsToHistorical(periods: any[]): HistoricalData[] {
  return periods.map(p => ({
    ...
    da: p.ebitda && p.ebit ? -(p.ebitda - p.ebit) : null,
    interestExpense: null,
    interestIncome: null,
    taxExpense: null,
    taxRate: p.net_income && p.ebit ? 1 - (p.net_income / p.ebit) : null,
    minorityInterests: null,
    eps: p.eps,
    ebitdaMargin: p.margin_ebitda,
    ebitMargin: p.ebit && p.revenue ? p.ebit / p.revenue : null,
    ...
    evFcf: p.p_fcf,
    evEbit: p.ebit && p.revenue ? null : null,
    marketCap: null,
    ev: null,
